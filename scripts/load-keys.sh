...
VOL=vuln-fix-agent_ssh-keys
docker volume create "$VOL" >/dev/null

# A throwaway container is the only way to write into a named volume from the host.
docker rm -f vuln-fix-agent-keyload >/dev/null 2>&1 || true
docker run -d --name vuln-fix-agent-keyload -v "$VOL":/ssh-keys alpine sleep 300 >/dev/null
...
for k in "$SSH_AUTH_KEY" "$SSH_SIGN_KEY_PRIVATE" "$SSH_SIGN_KEY"; do
  ...
  docker cp "$SSH_KEY_DIR/$k" vuln-fix-agent-keyload:/ssh-keys/"$k"
  echo "loaded $k"
done
